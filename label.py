...
def work(field):
    ...
    for obj in predictions:
        if obj['ground_truth']==-1 and obj['c1'] in concepts and obj['c2'] in concepts and obj['text_predict'][1]+obj['graph_predict'][1] > 1.6:
            filtered_list.append(obj)
    filtered_list.sort(key=lambda x: (x['text_predict'][1]+x['graph_predict'][1]), reverse=True)
    res = []
    for obj in random.sample(filtered_list, min(len(filtered_list), 4000)):
